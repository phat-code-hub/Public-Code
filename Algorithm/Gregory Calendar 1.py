...
def IsleapYear(year):
    if year % 400 == 0 or (year % 4 ==0  and year % 100 !=0 ):
        ...
        return False
#--------------------------------------------------------------
def day_of_week(year,month,opt_=1):
    if IsleapYear(year):
        days_of_month[2]=29
    ...
    return ordDays
#--------------------------------------------------------------
def show(year,month,dayOrd,start=1):
    print (f'{month}/{year:0>4} Calendar:')
    week_name=[w.rjust(3) for w in weekday_title[start-1]]
...
if __name__ == '__main__':
    ...
    opt=int(input('Starting weekday (1:Sunday ,2:Monday): '))
    print()
    show(year,month,day_of_week(year,month,opt),opt)